...
# 26 "mcc_generated_files/fatfs/drva.h"
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.36\\pic\\include\\c99\\stdbool.h" 1 3
# 26 "mcc_generated_files/fatfs/drva.h" 2
...
# 27 "mcc_generated_files/fatfs/drva.h" 2
...
)
{
    ...
    switch (pdrv) {
        ...
        case DRVA:
            if ( DRVA_IsMediaPresent() == 0)
            {
               stat = 0x02;
            }
            else if ( DRVA_IsMediaInitialized() == 1)
            {
                stat &= ~0x01;
            }

            if ( DRVA_IsWriteProtected() == 1)
            {
                stat |= 0x04;
            }
...
)
{
    ...
    switch (pdrv) {
        case DRVA :
            if(DRVA_SectorRead(sector, buff, count) == 1)
            {
                res = RES_OK;
            }
            else
...
)
{
    ...
    switch (pdrv) {
        case DRVA :
            if(DRVA_SectorWrite(sector, buff, count) == 1)
            {
                res = RES_OK;
            }
            else
